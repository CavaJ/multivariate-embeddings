 ...
 *  (default: 3)
 *  4. keep the original inner bag instance weights
 * </pre>
 * 
 * <pre>
 * -D
...
public class MIWrapper extends SingleClassifierEnhancer implements
  MultiInstanceCapabilitiesHandler, OptionHandler, TechnicalInformationHandler { // now implements WeightedInstancesHandler to be used with RF and LR
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    //System.out.println("\nPrinting instance weights after miToProp conversion...");
    //for(int i = 0; i < Math.min(10, train.numInstances()); i++)
    //{
    //  System.out.println(train.instance(i) + " => " + train.instance(i).weight());
    //} // for
  ...
  public double[] distributionForInstance(Instance exmp) throws Exception {
    ...
    // convert the training dataset into single-instance dataset
    //TODO interesting bagWeight/bagSize weighting regardless of the weighting scheme of training instances
    //m_ConvertToProp.setWeightMethod(new SelectedTag(
    //  MultiInstanceToPropositional.WEIGHTMETHOD_ORIGINAL,
    //  MultiInstanceToPropositional.TAGS_WEIGHTMETHOD));

    //keep the same weighting scheme as of training data
    m_ConvertToProp.setWeightMethod(getWeightMethod());
    ...
    for (int i = 0; i < nI; i++) {
      double[] dist = m_Classifier
        .distributionForInstance(testData.instance(i));
