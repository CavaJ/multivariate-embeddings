...
public class MIBoost extends SingleClassifierEnhancer implements OptionHandler,
  MultiInstanceCapabilitiesHandler, TechnicalInformationHandler { // now implements WeightedInstancesHandler to be used with RF and LR
  ...
  public void setWeightMethod(SelectedTag tag)
  {
    m_ConvertToSI.setWeightMethod(tag);
  } // setWeightMethod

  //getter for weight method
  public SelectedTag getWeightMethod()
  {
    return m_ConvertToSI.getWeightMethod();
  } // getWeightMethod
  ...
  public void buildClassifier(Instances exps) throws Exception {
    ...
    //by default it is method 3 of MultiInstanceToPropositional
    //m_ConvertToSI.setWeightMethod(new SelectedTag(MultiInstanceToPropositional.WEIGHTMETHOD_KEEP_INNER_BAG_INSTANCE_WEIGHTS, MultiInstanceToPropositional.TAGS_WEIGHTMETHOD));
    //new SelectedTag(MultiInstanceToPropositional.WEIGHTMETHOD_1, MultiInstanceToPropositional.TAGS_WEIGHTMETHOD)); //moved to a method
    m_ConvertToSI.setInputFormat(train);
    Instances data = Filter.useFilter(train, m_ConvertToSI);

    //TODO explicit unit-weighting
    ...
    //TODO INITIALIZING weights for train variable other than unit-weighting
    ...
    iterations: for (int m = 0; m < m_MaxIterations; m++) {
      ...
      for (int r = 0; r < N; r++)
      {
        ...
        //exr.setWeight(sumNi * exr.weight() / totWeights); // TODO original
        exr.setWeight(num * exr.weight() / totWeights); // TODO mine
