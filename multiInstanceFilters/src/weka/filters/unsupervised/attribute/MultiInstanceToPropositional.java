...
public class MultiInstanceToPropositional extends Filter implements
  OptionHandler, UnsupervisedFilter, MultiInstanceCapabilitiesHandler, WeightedInstancesHandler {
  ...
  /** weight method: keep the weight to be the same as the original value; it is actually bagWeight / bagSize */
  ...
  /**
   * weight method: keep the initial weight of every instance in the bag
   */
  public static final int WEIGHTMETHOD_KEEP_INNER_BAG_INSTANCE_WEIGHTS = 4;
  ...
  public static final Tag[] TAGS_WEIGHTMETHOD = {
    ...
    new Tag(WEIGHTMETHOD_KEEP_INNER_BAG_INSTANCE_WEIGHTS, "keep initial inner bag instance weights which sum up to 1")};
  ...
  private void convertInstance(Instance bag)
  {
    ...
    } else if (m_WeightMethod == WEIGHTMETHOD_ORIGINAL) {
      weight = bag.weight() / bagSize;
    }
    else //my own code
    {
      weight = 1.0; //initially 1 here, but updated in the loop below
    } // else
    ...
    for (int i = 0; i < bagSize; i++)
    {
      ...
      newInst.setWeight(weight); // if data is null the weight of only inner bag instance becomes 1.0 if WEIGHTMETHOD_KEEP_INNER_BAG_INSTANCE_WEIGHTS is selected

      if(data != null && m_WeightMethod == WEIGHTMETHOD_KEEP_INNER_BAG_INSTANCE_WEIGHTS) // otherwise, keep the initial inner bag weighting
        newInst.setWeight(data.instance(i).weight());
